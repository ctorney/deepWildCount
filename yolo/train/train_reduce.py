...
sys.path.append("..")
from yolo_models import get_yolo3
...
model = get_yolo3(IMAGE_W,IMAGE_H)
...
def custom_loss_old(y_true, y_pred):
 ...
 #   loss = loss_xy + loss_wh + loss_conf #+ loss_class
    ...
    loss = tf.Print(loss, [tf.shape(true_box_conf)], message='mask \t', summarize=1000)
...
def custom_loss(y_true, y_pred):
 ...
 #   pred_box_wh = tf.exp(0.*y_pred[..., 2:4])
    ...
    ### adjust confidence
    pred_box_conf = tf.sigmoid(y_pred[..., 1])
   ...
   # pred_box_class = y_pred[..., 5:]
    ...
    true_box_conf = iou_scores * y_true[..., 2]
   ...
   # true_box_class = tf.argmax(y_true[..., 5:], -1)
    ...
    best_ious = tf.expand_dims(best_ious, -1)
    ## CHANGED UP TO HERE
    conf_mask = conf_mask + tf.to_float(best_ious < 0.6) * (1 - y_true[..., 2]) * NO_OBJECT_SCALE
    
    # penalize the confidence of the boxes, which are reponsible for corresponding ground truth box
    conf_mask = conf_mask + y_true[..., 2] * OBJECT_SCALE
   ...
   # true_box_conf = y_true[..., 2]
    ...
    #loss = tf.Print(loss, [tf.shape(best_ious3)], message='true \t', summarize=1000)
    loss = tf.Print(loss, [tf.shape(conf_mask)], message='mask \t', summarize=1000)
    
    return loss
def custom_loss_new(y_true, y_pred):
    mask_shape = tf.shape(y_true)[:3]
